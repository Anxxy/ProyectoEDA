...
template<int limit>
class RTreeNodeRoutes;

template<int limit = 4>
class RTreeSubNodeRoutes {
  template<int>
  friend class RTreeNodeRoutes;

  using node_t = RTreeNodeRoutes<limit>;
  using subnode_t = RTreeSubNodeRoutes<limit>;
...
public:
  RTreeSubNodeRoutes(): id(""), next(nullptr), value(nullopt) {}
  RTreeSubNodeRoutes(string id, type_t&& value): id(id), next(nullptr), value(move(value)) {}
...
class RTreeNodeRoutes {
  ...
  template<int>
  friend class RTreeSubNodeRoutes;
...
public:
  RTreeNodeRoutes(string id, type_t&& value): is_leaf(true) {
    subNodes = new vector<subnode_t*>;
    bg::envelope(value, this->box);
    subNodes->push_back(new subnode_t(id, move(value)));
  }

  RTreeNodeRoutes(): is_leaf(true), subNodes(new vector<subnode_t*>) {}
  ...
  auto numberOfRoutes() -> int {
    ...
    for(auto sub : *subNodes) {
      if (sub->value.has_value()){
        ...
        if(sub->next) {
          result +=  sub->next->numberOfRoutes();
        }
      }
    }
    return result;
  ...
  auto getRoutes() -> map<string, type_t> {
    map<string, type_t> result;
    for(auto sub : *subNodes) {
      if (sub->value.has_value()){
        result.insert( {sub->id, sub->getPolygon()} );
      } else {
        if(sub->next) {
          auto temp = sub->next->getRoutes();
  ...
  auto getRoutesCircle(double x, double y, double r) -> map<string, type_t> {
    map<string, type_t> result;
    for(auto sub : *subNodes) {
      if (sub->value.has_value()){
        auto poly = sub->getPolygon();
          if(bg::distance(point_t(x, y),poly.outer()[0]) <= r && bg::distance(point_t(x, y),poly.outer()[1]) <= r){
            result.insert( {sub->id, sub->getPolygon()} );
          }
      } else {
        if(sub->next) {
